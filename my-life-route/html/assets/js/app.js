...
liferoute.controller('loginCtrl', function($scope, $window , loginService) {
    ...
    {
        console.log("Inside sign function ");

        console.log("Form data values", $scope.user);

        loginService.logIn($scope.user,function(er,rs){
...
liferoute.service('loginService', function ($http) {
    this.logIn = function(payload, cb){
        $http.post('http://trained.ignivastaging.com:8029/v1/Trainee/login', payload).then(function(success) {
            cb(null, success)
        },function(error){
            cb(error)
        });
